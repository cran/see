...
plot.see_parameters_sem <- function(
  ...
  # Plot Correlations
  if ("correlation" %in% component && "Correlation" %in% x$edges$Component) {
    p <- p +
  ...
  # Plot Loadings
  if ("loading" %in% component && "Loading" %in% x$edges$Component) {
    p <- p +
  ...
  # Plot regressions
  if ("regression" %in% component && "Regression" %in% x$edges$Component) {
    p <- p +
      ggraph::geom_edge_link(
        aes(
