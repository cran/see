...
data_plot.performance_pp_check <- function(x, type = "density", ...) {
  # 1. Handle data from bayesplot ----------------------------------------------
  # If the data comes from "bayesplot::pp_check()", it is already in the correct
  # shape. We just assign classes and attributes and return early.
  ...
  # 2. Prepare ordinal models --------------------------------------------------
  # For ordinal models, the simulated response might be numeric, while the
  # original response is an ordered factor. We fix the simulations to match here.
  ...
  # Ensure factor values are treated as characters to avoid grouping issues
  if (is.factor(dataplot[["values"]])) {
    dataplot[["values"]] <- as.character(dataplot[["values"]])
  }

  # Drop the last column (usually the 'id' column generated by reshape)
  dataplot <- dataplot[, 1:(ncol(dataplot) - 1), drop = FALSE]

  # Relabel the keys for clearer plotting legends
  dataplot$key[dataplot$key != "y"] <- "Model-predicted data"
  dataplot$key[dataplot$key == "y"] <- "Observed data"

  # Create a grouping variable tying the long rows back to original columns
...
print.see_performance_pp_check <- function(
  ...
  # Build and append range plot if requested
...
.prepare_pp_check <- function(
  ...
  # Save original object for the range plot
  ...
  # Ensure the data is prepared for plotting
...
.plot_pp_check <- function(
  ...
  # 1. Dispatch for Stan objects -----------------------------------------------
  # Discrete plot type from "bayesplot::pp_check()" returns a different data
  # structure, so we pass it to a dedicated handler
  ...
  # Default bandwidth for smoothing density curves
  bandwidth <- info$bandwidth
  if (is.null(bandwidth)) {
    bandwidth <- "nrd"
  }

  # Check if model properties suggest using discrete dots instead of lines
  ...
  if (
    ...
    # Discrete dot plots (typically for integer/count outcomes)
    ...
    # Warn user if they are using density plots for discrete data
    if (suggest_dots) {
    ...
    # Standard continuous density plot
  ...
  # Adjust legend layout if required
...
.plot_check_predictions_density <- function(
  ...
  # Generates standard overlaid density curves
  ggplot2::ggplot(x) +
...
.plot_check_predictions_dots <- function(
  ...
  # 1. Data Aggregation and Reshaping ------------------------------------------
  # Force factor so "table()" generates frequencies for all levels (including 0 counts).
  # We need tables of same size to correctly bind data frames.
  ...
  x$key[nrow(x)] <- "Observed data" # The last row is always the original observed data

  # Convert aggregated wide data back to long format for ggplot
  ...
  # 2. Build Error Bars Layer (if requested) -----------------------------------
  if (!is.null(type) && type %in% c("discrete_interval", "discrete_both")) {
    # Helper to calculate median and confidence intervals
  ...
  # 3. Build Raw Dots Layer (if requested) -------------------------------------
  if (!is.null(type) && type %in% c("discrete_dots", "discrete_both")) {
    ...
    p2 <- p2 +
      ...
      # Hidden layer specifically to force the legend behavior
      ggplot2::geom_point(
...
.plot_check_predictions_stan_dots <- function(
  ...
  # Normalize group names to match our established color scales
...
.plot_pp_check_range <- function(
  ...
  # Calculate min and max for the original observed data
  original <- data.frame(
    x = c(min(x$y), max(x$y)),
  ...
  # Calculate min and max for the replicated/simulated data runs
  replicated <- rbind(
    data.frame(
  ...
  # Choose geometric shape based on the number of unique integer breaks
